...
export function PixelWarrior({ strength, level, size = 320 }: PixelWarriorProps) {
  ...
  const renderDetailedWarrior = () => {
    ...
    if (level >= 6) {
      const capePixels = [
        ...
        [16,13],[17,13],[18,13],[19,13],[20,13],[21,13],[22,13],[23,13],[24,13],[25,13],[26,13],[27,13],[28,13],[29,13],
        [16,14],[17,14],[18,14],[28,14],[29,14],
        [17,15],[18,15],[28,15],[29,15],
        [17,16],[18,16],[28,16],[29,16],
        [18,17],[19,17],[27,17],[28,17],
        [18,18],[19,18],[27,18],[28,18],
        [19,19],[20,19],[26,19],[27,19],
        [19,20],[20,20],[26,20],[27,20],
        [20,21],[21,21],[25,21],[26,21],
        [20,22],[21,22],[25,22],[26,22],
        [21,23],[22,23],[24,23],[25,23],
      ];
      capePixels.forEach(([x, y], i) => {
        const isShadow = x < 22 || (y > 16 && x < 23);
        pixels.push(p(x, y, isShadow ? colors.capeShadow : colors.cape, `cape-${i}`));
      });
    }
    ...
    const hairPixels = [
      ...
      [18,7],[19,7],[20,7],[21,7],[22,7],[23,7],[24,7],[25,7],[26,7],[27,7],[28,7],
      [18,8],[19,8],[20,8],[27,8],[28,8],
      [18,9],[19,9],[28,9],[29,9],
    ];
    hairPixels.forEach(([x, y], i) => {
      const isHighlight = y === 6 && (x === 22 || x === 24);
      const isShadow = y >= 8 || x === 18 || x === 28;
      pixels.push(p(x, y, isHighlight ? colors.hairHighlight : isShadow ? colors.hairShadow : colors.hair, `hair-${i}`));
    });
    ...
    if (gender === 'female') {
      ...
      femHairPixels.forEach(([x, y], i) => {
        pixels.push(p(x, y, y > 10 ? colors.hairShadow : colors.hair, `hair-fem-${i}`));
      });
    }
    ...
    headPixels.forEach(([x, y], i) => {
      ...
      pixels.push(p(x, y, isHighlight ? colors.skinHighlight : isShadow ? colors.skinShadow : colors.skin, `head-${i}`));
    });

    // Eyes - detailed
    [[20,9],[21,9],[24,9],[25,9]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.white, `eye-white-${i}`));
    });
    [[21,10],[25,10]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.black, `eye-pupil-${i}`));
    });
    [[21,11],[25,11]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.black, `eye-lash-${i}`));
    });

    // Eyebrows
    [[20,8],[21,8],[24,8],[25,8]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.hairShadow, `eyebrow-${i}`));
    });

    // Nose
    [[22,10],[23,10],[24,10]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.skinShadow, `nose-${i}`));
    });

    // Mouth - smile
    [[21,11],[22,11],[23,11],[24,11],[25,11]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.skinDark, `mouth-${i}`));
    });
    [[22,12],[23,12],[24,12]].forEach(([x, y], i) => {
      pixels.push(p(x, y, colors.red, `mouth-inner-${i}`));
    });

    // Neck
    [[21,13],[22,13],[23,13],[24,13],[25,13]].forEach(([x, y], i) => {
      pixels.push(p(x, y, i < 2 || i > 2 ? colors.skinShadow : colors.skin, `neck-${i}`));
    });
    ...
    if (level >= 5) {
      ...
      helmetPixels.forEach(([x, y], i) => {
        ...
        pixels.push(p(x, y, isHighlight ? colors.armorHighlight : isShadow ? colors.armorShadow : colors.armor, `helmet-${i}`));
      });

      // Helmet plume
      [[22,5],[23,5],[24,5]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.red, `plume-${i}`));
      });
      [[22,4],[23,4],[24,4]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.red, `plume-top-${i}`));
      });

      // Visor
      [[20,9],[21,9],[22,9],[23,9],[24,9],[25,9],[26,9]].forEach(([x, y], i) => {
    ...
    if (level >= 6) {
      const crownPixels = [
        ...
        [19,3],[21,3],[23,3],[25,3],[27,3],
      ];
      crownPixels.forEach(([x, y], i) => {
        const isJewel = y === 3;
        const isHighlight = y === 4 && x % 2 === 1;
        pixels.push(p(x, y, isJewel ? colors.ultimate : isHighlight ? colors.goldHighlight : colors.gold, `crown-${i}`));
      });
    }
    ...
    const bodyPixels = [
      ...
      [18,16],[19,16],[20,16],[21,16],[22,16],[23,16],[24,16],[25,16],[26,16],[27,16],[28,16],
      [18,17],[19,17],[20,17],[21,17],[22,17],[23,17],[24,17],[25,17],[26,17],[27,17],[28,17],
      [19,18],[20,18],[21,18],[22,18],[23,18],[24,18],[25,18],[26,18],[27,18],
    ...
    bodyPixels.forEach(([x, y], i) => {
      ...
      pixels.push(p(x, y, isHighlight ? colors.clothingHighlight : isShadow ? colors.clothingShadow : colors.clothing, `body-${i}`));
    });

    // Belt (Level 2+)
    if (level >= 2) {
      [[19,20],[20,20],[21,20],[22,20],[23,20],[24,20],[25,20],[26,20],[27,20]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.darkBrown, `belt-${i}`));
      });
      [[22,20],[23,20],[24,20]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 ? colors.gold : colors.goldShadow, `buckle-${i}`));
      });
    }
    ...
    if (level >= 4) {
      const chestArmorPixels = [
        [20,15],[21,15],[25,15],[26,15],
        [20,16],[21,16],[25,16],[26,16],
        [21,17],[22,17],[24,17],[25,17],
        [22,18],[23,18],[24,18],
      ];
      chestArmorPixels.forEach(([x, y], i) => {
        const isHighlight = y === 15;
        pixels.push(p(x, y, isHighlight ? colors.armorHighlight : colors.armor, `chest-armor-${i}`));
      });
    }
    ...
    if (level >= 5) {
      ...
      platePixels.forEach(([x, y], i) => {
        ...
        pixels.push(p(x, y, isHighlight ? colors.armorHighlight : isShadow ? colors.armorShadow : colors.armor, `plate-${i}`));
      });

      // Armor details
      [[21,16],[25,16],[22,17],[24,17],[23,18]].forEach(([x, y], i) => {
    ...
    leftArmPixels.forEach(([x, y], i) => {
      const isShadow = x <= 14;
      pixels.push(p(x, y, isShadow ? colors.skinShadow : colors.skin, `arm-left-${i}`));
    });
    ...
    rightArmPixels.forEach(([x, y], i) => {
      const isShadow = x >= 32;
      pixels.push(p(x, y, isShadow ? colors.skinShadow : colors.skin, `arm-right-${i}`));
    });

    // Shoulder armor (Level 4+)
    if (level >= 4) {
      [[17,14],[18,14],[19,14]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 ? colors.armorHighlight : colors.armor, `shoulder-left-${i}`));
      });
      [[27,14],[28,14],[29,14]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 ? colors.armorHighlight : colors.armor, `shoulder-right-${i}`));
      });
    }

    // Gauntlets (Level 5+)
    if (level >= 5) {
      [[13,20],[14,20],[13,21],[14,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i % 2 === 0 ? colors.armor : colors.armorShadow, `gauntlet-left-${i}`));
      });
      [[32,20],[33,20],[32,21],[33,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i % 2 === 0 ? colors.armor : colors.armorShadow, `gauntlet-right-${i}`));
      });
    }
    ...
    if (level >= 3) {
      ...
      shieldPixels.forEach(([x, y], i) => {
        ...
        pixels.push(p(x, y, isHighlight ? colors.shieldHighlight : isShadow ? colors.shieldShadow : colors.shield, `shield-${i}`));
      });

      // Shield rim
      [[8,17],[9,17],[10,17],[11,17]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.armorShadow, `shield-rim-top-${i}`));
      });

      // Shield emblem (Level 4+)
      if (level >= 4) {
        [[9,20],[10,20]].forEach(([x, y], i) => {
          pixels.push(p(x, y, colors.gold, `emblem-${i}`));
        });
        [[9,21],[10,21]].forEach(([x, y], i) => {
    ...
    if (level === 2) {
      [[34,18],[34,19],[34,20],[34,21],[34,22],[34,23],[34,24]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i % 2 === 0 ? colors.brown : colors.darkBrown, `stick-${i}`));
      });
    }
    ...
    if (level >= 3 && level < 5) {
      // Handle
      [[34,19],[34,20],[34,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.darkBrown, `sword-handle-${i}`));
      });
      // Guard
      [[33,21],[34,21],[35,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, colors.armor, `sword-guard-${i}`));
      });
      // Blade
      [[34,22],[34,23],[34,24],[34,25],[34,26]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 ? colors.silverHighlight : colors.silver, `sword-blade-${i}`));
      });
    }
    ...
    if (level >= 5) {
      ...
      // Handle
      [[34,19],[34,20],[34,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 ? colors.darkBrown : colors.brown, `handle-${i}`));
      });
      // Guard
      [[33,21],[34,21],[35,21],[36,21]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 1 || i === 2 ? colors.goldHighlight : colors.gold, `guard-${i}`));
      });
      // Blade
      [[34,22],[34,23],[34,24],[34,25],[34,26],[34,27],[34,28]].forEach(([x, y], i) => {
        pixels.push(p(x, y, i === 2 || i === 4 ? colors.silverHighlight : colors.silver, `blade-${i}`));
      });
      [[34,29]].forEach(([x, y], i) => {
    ...
    // === LEGS ===
    const legPixels = [
      [21,22],[22,22],[24,22],[25,22],
      [21,23],[22,23],[24,23],[25,23],
      [21,24],[22,24],[24,24],[25,24],
      [21,25],[22,25],[24,25],[25,25],
      [21,26],[22,26],[24,26],[25,26],
      [21,27],[22,27],[24,27],[25,27],
    ];
    legPixels.forEach(([x, y], i) => {
      const isShadow = x === 25 || y >= 26;
      pixels.push(p(x, y, isShadow ? colors.clothingShadow : colors.clothing, `leg-${i}`));
    });

    // Boots
    const bootPixels = [
      [20,28],[21,28],[22,28],[24,28],[25,28],[26,28],
      [20,29],[21,29],[22,29],[24,29],[25,29],[26,29],
      [20,30],[21,30],[22,30],[23,30],[24,30],[25,30],[26,30],
    ];
    bootPixels.forEach(([x, y], i) => {
      pixels.push(p(x, y, i % 3 === 0 ? colors.darkBrown : colors.brown, `boot-${i}`));
    });
    ...
    if (level >= 6) {
      // Aura particles
      const particles = [
        [12,10],[35,10],[10,14],[36,14],[13,24],[34,24],[15,28],[32,28],
        [11,6],[35,6],[8,12],[38,12],[6,20],[40,20],[10,30],[36,30],
      ];
      particles.forEach(([x, y], i) => {
        pixels.push(p(x, y, i % 2 === 0 ? colors.gold : colors.glow, `particle-${i}`));
      });

      // Sparkles
      const sparkles = [
        [14,8],[33,8],[12,16],[35,16],[16,25],[31,25],
      ];
